...
class SmallPizza(Pizza):
    ...
    def __init__(self, ingrediets, size = "small", base_price = 10):
        super().__init__( size, ingrediets, base_price)

class MediumPizza(Pizza):

    def __init__(self,ingrediets,  size = "medium", base_price = 15):
        super().__init__( size, ingrediets, base_price)

class LargePizza(Pizza):

    def __init__(self, ingrediets, size = "large", base_price = 20):
        super().__init__( size, ingrediets, base_price)
...
class PizzaOrder(Order):
    ...
    def __init__(self, pizza: Pizza):
        self._pizza = pizza
    def prepare(self):
        print(f"Готовим пиццу {self._pizza.__str__()} {self._pizza._size}: Цена {self._pizza.calculete_price()}$")

pizza = LargePizza(ingrediets=['Сыр', 'Грибы', 'Ветчина'])
